import baca

distances = [20, (34), (31, 31, 34), (31, 31, 31, 31, 34)]

breaks = baca.breaks(
    baca.page(
        baca.system(
            *[22, (28), (25, 25, 28), (25, 25, 25, 25, 28)],
            measure=1,
            y_offset=90,
        ),
        number=1,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=5,
            measure=4,
            y_offset=32,
        ),
        number=2,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=9,
            measure=8,
            y_offset=32,
        ),
        number=3,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=13,
            measure=12,
            y_offset=32,
        ),
        number=4,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=17,
            # measure=16,
            # measure=17,
            measure=16,
            y_offset=32,
        ),
        number=5,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=21,
            y_offset=32,
        ),
        number=6,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=25,
            y_offset=32,
        ),
        number=7,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=29,
            y_offset=32,
        ),
        number=8,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=33,
            measure=34,
            y_offset=32,
        ),
        number=9,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=37,
            y_offset=32,
        ),
        number=10,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=41,
            measure=40,
            y_offset=32,
        ),
        number=11,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=45,
            y_offset=32,
        ),
        number=12,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=49,
            y_offset=32,
        ),
        number=13,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=53,
            y_offset=32,
        ),
        number=14,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=57,
            measure=58,
            y_offset=32,
        ),
        number=15,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=61,
            y_offset=32,
        ),
        number=16,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=65,
            y_offset=32,
        ),
        number=17,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=70,
            measure=68,
            y_offset=32,
        ),
        number=18,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=73,
            y_offset=32,
        ),
        number=19,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=77,
            y_offset=32,
        ),
        number=20,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=81,
            y_offset=32,
        ),
        number=21,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=85,
            y_offset=32,
        ),
        number=22,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=89,
            y_offset=32,
        ),
        number=23,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=93,
            measure=94,
            y_offset=32,
        ),
        number=24,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=98,
            y_offset=32,
        ),
        number=25,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=101,
            y_offset=32,
        ),
        number=26,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=105,
            y_offset=32,
        ),
        number=27,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=109,
            y_offset=32,
        ),
        number=28,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=113,
            y_offset=32,
        ),
        number=29,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=117,
            y_offset=32,
        ),
        number=30,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=121,
            y_offset=32,
        ),
        number=31,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=125,
            y_offset=32,
        ),
        number=32,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=130,
            y_offset=32,
        ),
        number=33,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=133,
            y_offset=32,
        ),
        number=34,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=137,
            y_offset=32,
        ),
        number=35,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=141,
            y_offset=32,
        ),
        number=36,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=145,
            y_offset=32,
        ),
        number=37,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=149,
            y_offset=32,
        ),
        number=38,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=154,
            y_offset=32,
        ),
        number=39,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=157,
            y_offset=32,
        ),
        number=40,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=161,
            y_offset=32,
        ),
        number=41,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=165,
            y_offset=32,
        ),
        number=42,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=169,
            y_offset=32,
        ),
        number=43,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=173,
            y_offset=32,
        ),
        number=44,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=177,
            measure=178,
            y_offset=32,
        ),
        number=45,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=181,
            y_offset=32,
        ),
        number=46,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=185,
            y_offset=32,
        ),
        number=47,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=189,
            y_offset=32,
        ),
        number=48,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=193,
            measure=194,
            y_offset=32,
        ),
        number=49,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=197,
            y_offset=32,
        ),
        number=50,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=201,
            y_offset=32,
        ),
        number=51,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=205,
            y_offset=32,
        ),
        number=52,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=209,
            y_offset=32,
        ),
        number=53,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=213,
            y_offset=32,
        ),
        number=54,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=217,
            y_offset=32,
        ),
        number=55,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=221,
            y_offset=32,
        ),
        number=56,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=225,
            y_offset=32,
        ),
        number=57,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=229,
            # measure=230,
            measure=228,
            y_offset=32,
        ),
        number=58,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=233,
            measure=232,
            y_offset=32,
        ),
        number=59,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=237,
            measure=235,
            y_offset=32,
        ),
        number=60,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=241,
            measure=240,
            y_offset=32,
        ),
        number=61,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=245,
            y_offset=32,
        ),
        number=62,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=249,
            y_offset=32,
        ),
        number=63,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=253,
            # measure=255,
            # measure=253,
            measure=252,
            y_offset=32,
        ),
        number=64,
    ),
    baca.page(
        baca.system(
            *distances,
            # measure=259,
            # measure=260,
            measure=256,
            y_offset=32,
        ),
        number=65,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=260,
            y_offset=32,
        ),
        number=66,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=263,
            y_offset=32,
        ),
        number=67,
    ),
    baca.page(
        baca.system(
            *distances,
            measure=266,
            y_offset=32,
        ),
        number=68,
    ),
)

spacing = baca.scorewide_spacing(
    __file__,
    breaks=breaks,
    fallback_duration=(1, 40),
)

text_measure_numbers = [
    15,
    18,
    20,
    41,
    48,
    57,
    69,
    82,
    91,
    97,
    107,
    115,
    119,
    122,
    129,
    156,
    163,
    170,
    177,
    193,
    206,
    229,
    236,
    239,
    248,
    254,
    261,
    268,
]

eol_fermata_spacing = (1, 132)
mol_fermata_spacing = (1, 172)

# spacing.override(text_measure_numbers, (1, 288), fermata=True)
# spacing.override(text_measure_numbers, (1, 144), fermata=True)
spacing.override(text_measure_numbers, eol_fermata_spacing, fermata=True)

spacing.override(11, (1, 20))

# spacing.override([12, 13, 14], (1, 16))
spacing.override([12, 13, 14], (1, 20))
# spacing.override([16, 17, 19], (1, 20))
# spacing.override([16, 17, 19], (1, 16))
# spacing.override([16, 19], (1, 16))
spacing.override(19, (1, 16))

# spacing.override(18, (1, 144))
# spacing.override(18, (1, 188))
# spacing.override(18, (1, 166))
spacing.override(18, mol_fermata_spacing)


spacing.override(41, mol_fermata_spacing)
# spacing.override([40, 42, 43, 44], (1, 32))
# spacing.override([40, 42, 43, 44], (1, 20))
spacing.override([40, 42, 43, 44], (1, 16))

spacing.override((45, 47), (1, 36))
# spacing.override((53, 56), (1, 36))
# spacing.override((53, 56), (1, 32))
spacing.override((53, 56), (1, 20))

spacing.override(69, mol_fermata_spacing)
spacing.override([68, 70, 71, 72], (1, 20))

spacing.override(82, mol_fermata_spacing)
spacing.override([81, 83, 84], (1, 20))

spacing.override(91, mol_fermata_spacing)
spacing.override([90, 92, 93], (1, 20))
spacing.override((94, 96), (1, 32))

spacing.override(107, mol_fermata_spacing)
spacing.override([105, 106, 108], (1, 20))

spacing.override(115, mol_fermata_spacing)
spacing.override([113, 114, 116], (1, 20))

spacing.override(119, mol_fermata_spacing)
spacing.override([117, 118, 120], (1, 32))

spacing.override(122, mol_fermata_spacing)
# spacing.override([121, 123, 124], (1, 32))
spacing.override([121, 123, 124], (1, 24))
# spacing.override((125, 128), (1, 32))
spacing.override((125, 128), (1, 24))

spacing.override(163, mol_fermata_spacing)
# spacing.override([161, 162, 164], (1, 32))
spacing.override([161, 162, 164], (1, 24))

spacing.override(170, mol_fermata_spacing)
# spacing.override([169, 171, 172], (1, 32))
spacing.override([169, 171, 172], (1, 24))
# spacing.override((173, 176), (1, 32))
spacing.override((173, 176), (1, 20))
spacing.override((189, 192), (1, 20))

spacing.override(206, mol_fermata_spacing)
spacing.override([205, 207, 208], (1, 32))
# spacing.override((226, 227), (1, 20))
### spacing.override((225, 228), (1, 20))
# spacing.override((233, 235), (1, 32))
spacing.override(229, mol_fermata_spacing)
spacing.override([228, 230, 231], (1, 20))

### spacing.override((233, 235), (1, 20))
### spacing.override(239, mol_fermata_spacing)
# spacing.override([237, 238, 240], (1, 32))
### spacing.override([237, 238, 240], (1, 24))
spacing.override(236, mol_fermata_spacing)
spacing.override(235, (1, 20))
spacing.override([237, 238], (1, 12))

# spacing.override((246, 247), (1, 20))
spacing.override((245, 247), (1, 20))

spacing.override(254, mol_fermata_spacing)
spacing.override(255, (1, 12))

spacing.override((256, 259), (1, 56))

spacing.override(261, mol_fermata_spacing)
spacing.override(262, (1, 24))

spacing.override((266, 267), (1, 24))
spacing.override(268, mol_fermata_spacing)
